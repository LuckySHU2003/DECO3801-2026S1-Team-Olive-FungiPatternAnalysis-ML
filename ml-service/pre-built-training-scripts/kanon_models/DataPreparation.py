...
def detect_columns(df):
    ...
    # define first column as time column and remaining columns as voltage columns
...
def noise_reduction(df, window_val=5): 
    ...
    # apply rolling mean to each voltage column
    for col in voltage_col:
...
def extract_features_from_data(df, time_col, voltage_col, window_size, step):
    ...
    # if the dataset is too small to extract features, return an empty dataframe
    ...
    for start in range(0, len(df) - window_size + 1, step):
        ...
        # skip windows with NaN values or zero standard deviation
        if np.isnan(signal).any():
            continue
        if np.std(signal) == 0:
            continue

        # standardise the signal and find peaks
        z = (signal - np.mean(signal)) / np.std(signal)
        threshold = np.percentile(z, 80)
        peaks, _ = find_peaks(z, height=threshold)
        peak_values = signal[peaks]

        # calculate features and extracted features
